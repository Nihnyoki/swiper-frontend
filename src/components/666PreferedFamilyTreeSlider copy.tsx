  ...
  const fetchPeopleTree = async function (start: boolean): Promise<Person[]> {
    ...
    try {
      ...
      if (people.length > 0) {
        ...
        //setSecondActiveIndex(secondActiveIndex)
...
useEffect(() => {
  ...
  return (
    <div className=" flex flex-col items-center gap-1 w-full h-full bg-pink-100 rounded-xl mx-auto relative">
      {/* : MAIN + Content */}
      <div className="flex w-full h-[15%] gap-1 justify-center">
        {/* MAIN slider container */}
        <div className="w-full h-full relative pointer-events-auto">
          {people[firstActiveIndex] && people[firstActiveIndex].THINGS[secondActiveIndex] && (
            <Swiper
              key={firstActiveIndex}
              direction="vertical"
              slidesPerView={1}
              onSlideChange={(swiper) =>
                handleVerticalChange(swiper, people[swiper.activeIndex])
              }
              allowTouchMove={true}
        ...
        <div className="flex w-full h-full z-10 relative pointer-events-auto">
          {people[firstActiveIndex].THINGS[secondActiveIndex] && people[firstActiveIndex].THINGS[secondActiveIndex].val && (
            <Swiper
      ...
      {/* : Children */}
      <div className="flex-col w-full h-full overflow-y-auto flex-1 pb-4 z-10 relative pointer-events-auto">
